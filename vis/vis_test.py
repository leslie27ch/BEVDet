...
def main():
    ...
    for cnt, infos in enumerate(
        ...
        if len(pred_boxes) == 0:
            ...
            #test
            import math
            t = np.array([[0,0,0,10,2,1,math.pi/2]])
            boxest = LB(t, origin=(0.5, 0.5, 0.0))
            # a = boxest.corners.numpy()
            corners_lidar = boxest.corners.numpy().reshape(-1, 3)
        ...
        head_bev = corners_lidar[:, [0, 3], :2].mean(axis=1)
        canter_canvas = \
        ...
        color = color_map[int(pred_flag[0])]
        for index in draw_boxes_indexes_bev:
            cv2.line(
                canvas,
                tuple(bottom_corners_bev[0, index[0]]),
                tuple(bottom_corners_bev[0, index[1]]),
                tuple([color[0] * 1, color[1] * 1, color[2] * 1]),
                thickness=1)
        cv2.line(
            canvas,
            tuple(center_canvas[0]),
            tuple(head_canvas[0]),
            tuple([color[0] * 1, color[1] * 1, color[2] * 1]),
            1,
            lineType=8)
